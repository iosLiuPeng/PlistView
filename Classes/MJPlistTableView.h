...
@class MJPlistTableView;

@protocol MJPlistTableViewDelegate <NSObject>
@optional
/**
 额外配置cell
 少数情况下会用到，一般需要做附加设置，只需在对应cell里面重写configCellWithData方法，并且在方法中调用[super configCellWithData]
 */
- (void)plistTableView:(MJPlistTableView *)plistTableView configCell:(UITableViewCell *)aCell withKey:(NSString *)key;
@end

@interface MJPlistTableView : UITableView
...
@property (nonatomic, weak) IBOutlet id<MJPlistTableViewDelegate> customDlegate;///< 自定义方法代理 (可在xib面板中连线到File Owner)
